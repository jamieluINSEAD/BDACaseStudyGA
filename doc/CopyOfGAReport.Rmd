...
The top visited pages are 1) /como-crear-una-plantilla-php-desde-cero/ (2,903); 2) / (homepage) (1,981); 3) /aplicaciones-programas-gratuito-todas-necesidades-lista-software-libre/ (1,328). When the medium is not "cpc", the top two websites are the same but the third one is different. The most popular pages should not be advertised as it does not make a huge difference.
...
The exit rates for the pages with the most number of exits are the following: 1) 42%, 2) 45%, and 3) 61%. The content of the page should be changed so that it jives more with whatever the advertising in the search engine is advertising. 
...
The pages with the highest time on page are the following: 1) como-crear-una-plantilla-php-desde-cero/; 2) /; 3) /aplicaciones-programas-gratuito-todas-necesidades-lista-software-libre/. The author should focus on more on "how to" articles and curated lists of themes that people are interested in. However, the author should not necessarily just write about these topics because you want a breadth of topics.
...
The top sources are: 1) Google organic (1,979); 2) Google cpc (1,305); and 3) direct.
...
When the medium is referral, the sources that attract the highest number pageviews are: 1) semalt.semalt.com; 2) facebook.com; 3) linkedin.com. If easy, the author could promote content on facebook.com and linkedin.com so that advertising and content promotion isn't concentrated on just a couple sites. However, it's better to focus on investing in visitors from the medium 'cpc'.